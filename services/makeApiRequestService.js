...
const makeApiRequest = async () => {
    try {
        const response = await instance.post("https://localhost:1338/item",{timeout:5000});
        eapts_sync_logger.info("Items updated successfully",response);
    } catch (err) {
        eapts_sync_logger.error("Error updating items:", err);
    }
    try {
        const response = await instance.post("https://localhost:1338/dispensing-unit",{timeout:5000});
